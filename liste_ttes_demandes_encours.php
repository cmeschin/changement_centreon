...
try {
	include_once('requete_liste_ttes_demande_encours.php');
} catch (Exception $e) {
	http_response_code(500);
	echo '<p>Erreur requete liste ttes demande encours: ' . $e->getMessage() . '<p/>';
	die('Erreur requete liste ttes demande encours: ' . $e->getMessage());
};
try {
	include_once('requete_liste_ttes_demande_encours_temps_global.php');
} catch (Exception $e) {
	http_response_code(500);
	echo '<p>Erreur requete liste ttes demande encours temps global: ' . $e->getMessage() . '<p/>';
	die('Erreur requete liste ttes demande encours temps global: ' . $e->getMessage());
};
...
// 					} elseif ((htmlspecialchars($res_dem['Etat_Demande']) == "A Traiter") && (htmlspecialchars($res_etat['Etat_Dem']) == "En cours")) // si la demande est A traiter et le statut a insérer est "En cours" on désactive
// 					{
// 						echo '<option Disabled="Disabled" value="' . htmlspecialchars($res_etat['Etat_Dem']) . '">' . htmlspecialchars($res_etat['Etat_Dem']) . '</option> ';
// 					}
