 ...
 */
try {
	include_once('requete_liste_ttes_demandes_traite_par_mois.php');
} catch (Exception $e) {
	die('Erreur requete liste ttes demandes traite: ' . $e->getMessage());
};
try {
	include_once('requete_liste_ttes_demandes_traite_par_mois_temps_global.php');
} catch (Exception $e) {
	http_response_code(500);
	echo '<p>Erreur requete liste ttes demandes traite par mois temps global: ' . $e->getMessage() . '<p/>';
	die('Erreur requete liste ttes demandes traite par mois temps global: ' . $e->getMessage());
};
