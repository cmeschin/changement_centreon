...
try {
	include_once('requete_liste_mes_demandes_encours.php');
} catch (Exception $e) {
	http_response_code(500);
	echo '<p>Erreur requete liste mes demandes encours: ' . $e->getMessage() . '<p/>';
	die('Erreur requete liste mes demandes encours: ' . $e->getMessage());
};
try {
	include_once('requete_liste_mes_demandes_encours_temps_global.php');
} catch (Exception $e) {
	http_response_code(500);
	echo '<p>Erreur requete liste mes demandes encours temps global: ' . $e->getMessage() . '<p/>';
	die('Erreur requete liste mes demandes encours temps global: ' . $e->getMessage());
};
...
// 				{
// 					if (htmlspecialchars($res_etat['Etat_Dem']) != "En cours")
// 					{
