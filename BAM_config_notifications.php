...
//if ($sMonClient ) {
// // récupérer la liste de toutes les AM de centreon
// try {
// 	include_once('requete_BAM_liste_AM.php');
// } catch (Exception $e) {
// 	http_response_code(500);
// 	die('Erreur requete BAM liste AM: ' . $e->getMessage());
// };

//echo '<fieldset id="field_config_notification" class="Admin_Gestion_BAM">';
...
// 	echo '<label for="gb_prestation">Prestation :</label>';
// 	echo '<select name="gb_prestation" id="gb_prestation" onblur="verifChamp(this)" class="gb_prestation">';
// 	while ($res_lst_prestation = $req_lst_prestation->fetch())
// 	{
// 		echo '<option value="' . htmlspecialchars($res_lst_prestation['prestation']) . '">' . htmlspecialchars($res_lst_prestation['prestation']) . '</option> ';
// 	};
// 	echo '</select> <br />';
	...
	echo '</div>';
		...
		echo '<input type="text" id="gb_heure" name="gb_heure" oonclick="liste_heure();" size="5" onblur="verifChamp(this)" class="gb_heure gb_config"/>';
	...
	echo '</div>';
	echo '<button id="Envoie_Mail_BAM" onclick="Envoie_Mail_BAM();">Test envoi mail</button>';
